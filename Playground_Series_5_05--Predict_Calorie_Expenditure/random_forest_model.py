...
def make_prediction(model: RandomForestRegressor, test_df: pd.DataFrame,
                    skl_transformer: FunctionTransformer, cv_index: int | str) -> None:
    """ Make a prediction for the test data, with a given model.
    Args:
        model (RandomForestRegressor): Model used for the prediction.
    ...
    submit_df.to_csv(f"predictions_RF_KFold_{cv_index}.csv",
...
if NUM_CV_SPLITS > 1:  # do cross-validation
    ...
    for train_index, val_index in kfold.split(X):
        ...
        make_diagonal_plot(pd.DataFrame({TARGET_COL: y_train, 'PREDICTION': pred_train}),
                        ...
                        f"error_diagonal_RF_{cv_index}.png", precision=5)
